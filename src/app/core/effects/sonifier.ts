...
export class SonifierEffects {
  @Effect()
  headerLoaded$: Observable<Action> = this.actions$.pipe(
    ...
    flatMap(([action, dataFiles, imageFileStates]) => {
      ...
      if (dataFile.type == DataFileType.IMAGE) {
        ...
        let sourceExtractorState = imageFileStates[dataFile.id].sourceExtractor;
  ...
  viewportChanged$: Observable<Action> = this.actions$.pipe(
    ...
    map(
      action => new sonifierActions.UpdateRegion({ file: action.payload.file })
    )
  );

  @Effect()
  regionModeChanged$: Observable<Action> = this.actions$.pipe(
    ofType<sonifierActions.SetRegionMode>(sonifierActions.SET_REGION_MODE),
    map(
      action => new sonifierActions.UpdateRegion({ file: action.payload.file })
    )
  ...
  @Effect()
  regionHistoryChanged$: Observable<Action> = this.actions$.pipe(
    ofType<
    ...
    flatMap(([action, imageFileStates]) => {
      let sonifierFileState = imageFileStates[action.payload.file.id].sonifier;
      let actions: Action[] = [];
  ...
  @Effect()
  regionUpdated$: Observable<Action> = this.actions$.pipe(
  ...
  @Effect()
  updateSonifierUri$: Observable<Action> = this.actions$.pipe(
  ...
  constructor(
    ...
    private store: Store<fromCore.State> // @Optional() // @Inject(SEARCH_DEBOUNCE) // private debounce: number,
  ) // /**
