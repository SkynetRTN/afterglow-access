...
export class TransformationEffects {
  ...
  centerRegionInViewport$: Observable<Action> = this.actions$
    .ofType<transformationActions.CenterRegionInViewport>(transformationActions.CENTER_REGION_IN_VIEWPORT)
    .withLatestFrom(
    ...
    .flatMap(([action, imageFileStates]) => {
      ...
      if (!viewportSize) viewportSize = imageFileStates[imageFile.id].transformation.viewportSize;
      ...
      let yShift = viewportAnchor.y - scale * (getHeight(imageFile) - (region.y + region.height / 2));
      let viewportTransform = new Matrix(scale, 0, 0, scale, xShift, yShift);

      actions.push(new transformationActions.ResetImageTransform({ file: imageFile }))
      actions.push(new transformationActions.SetViewportTransform({ file: imageFile, transform: viewportTransform }))
  ...
  viewportChanged$: Observable<Action> = this.actions$
    ...
    .flatMap(([action, imageFileStates, workbenchState]) => {
      ...
      if (imageFile.headerLoaded && imageFileStates[action.payload.file.id].transformation.viewportSize) {
        ...
        if (sourceExtractor.regionOption == SourceExtractorRegionOption.VIEWPORT) actions.push(new sourceExtractorActions.UpdateRegion({ file: action.payload.file }));
      }
      return Observable.from(actions);
    });
