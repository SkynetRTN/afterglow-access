...
import { withLatestFrom, switchMap, map, flatMap } from "rxjs/operators";
...
export class WorkbenchEffects {
    ...
    .pipe(
      ...
      switchMap(([action, dataFiles, viewers, activeViewer]) => {
        ...
        if (
          ...
          actions.push(
            new workbenchActions.SelectDataFile({ file: dataFiles[0] })
          );
        }
        return from(actions);
      })
    ...
    .pipe(
      ...
      flatMap(([action, dataFiles, workbenchState, imageFileStates]) => {
        ...
        let pendingViewer = workbenchState.viewers.find(
          viewer => viewer.pendingFileId == action.payload.file.id
        );
        ...
        if (pendingViewer) {
          if (!dataFile.headerLoaded) {
            if (!dataFile.headerLoading)
              actions.push(
                new dataFileActions.LoadDataFileHdr({ file: dataFile })
              );
          } else if (!dataFile.histLoaded) {
            if (!dataFile.histLoading)
              actions.push(
                new imageFileActions.LoadImageHist({ file: dataFile })
              );
          } else {
    ...
    .pipe(
      ...
      flatMap(([action, dataFiles, workbenchState, imageFileStates]) => {
        ...
        let files = workbenchState.viewers
          .filter(
            (viewer, index) =>
              index != workbenchState.activeViewerIndex &&
              viewer.fileId !== null
          )
  ...
  syncFileNormalizations$: Observable<Action> = this.actions$
    ...
    .pipe(
      ...
      flatMap(([action, dataFiles, workbenchState, imageFileStates]) => {
        ...
        let srcNormalizer =
          imageFileStates[srcFile.id].normalization.normalizer;
  ...
  constructor(
    ...
    private store: Store<fromCore.State> // @Optional() // @Inject(SEARCH_DEBOUNCE) // private debounce: number, // /** //    * You inject an optional Scheduler that will be undefined //    * in normal application usage, but its injected here so that you can mock out
