import { Component, OnInit, HostListener, Input, OnDestroy, Output, EventEmitter, ChangeDetectionStrategy, ViewChild } from "@angular/core";
import { distinctUntilChanged, map, switchMap, takeUntil, tap, withLatestFrom } from "rxjs/operators";
import { MarkerType, Marker, CircleMarker } from "../../models/marker";
...
import { AddCustomMarkers, DeselectCustomMarkers, RemoveCustomMarkers, SelectCustomMarkers, SetCustomMarkerSelection, UpdateCustomMarker, UpdateCustomMarkerPanelConfig } from "../../workbench.actions";
...
export class CustomMarkerPanelComponent extends ToolPanelBaseComponent implements OnInit, OnDestroy {
  ...
  constructor(store: Store) {
    ...
    this.markerClickEvent$.pipe(
      ...
      if ($event.mouseEvent.ctrlKey) {
        ...
        this.store.dispatch(
          new SetCustomMarkerSelection(state.id, [customMarker])
        );
      }
    ...
    this.imageClickEvent$.pipe(
      takeUntil(this.destroy$),
      withLatestFrom(this.state$, this.config$, imageData$)
      ...
      if ($event.hitImage) {
        if (selectedCustomMarkers.length == 0 || $event.mouseEvent.altKey) {
          ...
          this.store.dispatch(
            new AddCustomMarkers(state.id, [customMarker])
          );
        } else {
          this.store.dispatch(
            new SetCustomMarkerSelection(state.id, [])
          );
        }
      }
    })
  ...
  deleteSelectedMarkers(markers: Marker[]) {
    if (!this.viewer || !this.state) return;
    this.store.dispatch(
      new RemoveCustomMarkers(this.state.id, markers)
    );
  }

  onCentroidClicksChange($event) {
    this.store.dispatch(new UpdateCustomMarkerPanelConfig({
