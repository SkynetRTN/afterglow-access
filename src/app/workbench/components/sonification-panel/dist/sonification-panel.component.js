...
var SonificationPanelComponent = /** @class */ (function () {
  function SonificationPanelComponent(
    afterglowService,
    _hotkeysService,
    ref,
    ...
    this.hotKeys = [];
    this.subs = [];
    this.region$ = rxjs_1.combineLatest(this.hdu$, this.transform$, this.viewportSize$, this.state$).pipe(
      ...
      operators_1.map(function (_a) {
        ...
        if (!hdu || !hdu.header.loaded || !transform || !viewportSize) {
          return null;
        }
        var rawImageData = _this.store.selectSnapshot(data_files_state_1.DataFilesState.getImageDataEntities)[
          hdu.rawImageDataId
        ];
        return transformation_1.getViewportRegion(
          transform,
          rawImageData.width,
          rawImageData.height,
          viewportSize.width,
    ...
    this.sonificationUri$ = this.region$.pipe(
      operators_1.map(function (region) {
        if (!region) return null;
        _this.sonificationUri = _this.afterglowDataFileService.getSonificationUri(
          _this.hdu.id,
          region,
          _this.state.duration,
          _this.state.toneCount
        );
    ...
    this.clearProgressLine$ = this.sonificationUri$.pipe(
      operators_1.filter(function (uri) {
      ...
      operators_1.tap(function () {
        return (_this.sonificationSrcUri = null);
      })
    );
    this.hotKeys.push(
      new angular2_hotkeys_1.Hotkey(
        "t 1",
        function (event) {
          if (_this.state.regionMode != sonifier_file_state_1.SonifierRegionMode.CUSTOM) return true;
    ...
    this.hotKeys.forEach(function (hotKey) {
      return _this._hotkeysService.add(hotKey);
    });
  }
  ...
  SonificationPanelComponent.prototype.ngOnDestroy = function () {
    var _this = this;
    this.hotKeys.forEach(function (hotKey) {
      return _this._hotkeysService.remove(hotKey);
    });
  };
  ...
  SonificationPanelComponent.prototype.selectSubregionByFrequency = function (subregion) {
    ...
    this.store.dispatch(
      new workbench_actions_1.AddRegionToHistory(this.hdu.id, {
  ...
  SonificationPanelComponent.prototype.setRegionMode = function ($event) {
    this.store.dispatch(
      new workbench_actions_1.UpdateSonifierFileState(this.hdu.id, {
  ...
  SonificationPanelComponent.prototype.sonify = function () {
    ...
    if (this.sonificationSrcUri == this.sonificationUri && this.api && this.api.canPlay) {
  ...
  SonificationPanelComponent.prototype.onPlayerReady = function (api) {
    ...
    this.subs.push(
      rxjs_1.from(this.api.getDefaultMedia().subscriptions.canPlayThrough).subscribe(function (canPlayThrough) {
        _this.loading = false;
      })
    );
    this.subs.push(
      rxjs_1.from(this.api.getDefaultMedia().subscriptions.loadStart).subscribe(function (canPlayThrough) {
        _this.loading = true;
      })
    );
    var indexToneDuration = 0.852 / 2.0;
    this.progressLine$ = rxjs_1.merge(
      start$.pipe(
        operators_1.flatMap(function () {
          return rxjs_1.interval(10).pipe(operators_1.takeUntil(rxjs_1.merge(stop$, _this.clearProgressLine$)));
        }),
        operators_1.withLatestFrom(this.region$),
        operators_1.map(function (_a) {
          var v = _a[0],
            region = _a[1];
          if (!_this.api.getDefaultMedia()) return null;
          if (!_this.api.getDefaultMedia().duration) return null;
          if (!region) return null;
          ...
          var y =
            ...
            Math.max(
              ...
              Math.min(
                1,
                (_this.api.getDefaultMedia().currentTime - indexToneDuration) /
    ...
    this.subs.push(
      this.progressLine$.pipe(operators_1.distinctUntilChanged()).subscribe(function (line) {
        _this.store.dispatch(new workbench_actions_1.SetProgressLine(_this.hdu.id, line));
      })
    );
  };
  __decorate([core_1.Input("hdu")], SonificationPanelComponent.prototype, "hdu");
  __decorate([core_1.Input("transform")], SonificationPanelComponent.prototype, "transform");
  __decorate([core_1.Input("viewportSize")], SonificationPanelComponent.prototype, "viewportSize");
  __decorate([core_1.Input("state")], SonificationPanelComponent.prototype, "state");
