...
export class HduSelectorComponent implements OnInit {
  ...
  constructor(private store: Store) {
    this.hdus$ = this.fileId$.pipe(
      switchMap((fileId) =>
        this.store.select(DataFilesState.getFileById).pipe(
          map((fn) => fn(fileId)),
          map((file) => {
  ...
  onSelectedHduIdChange($event: MatSelectChange) {
    ...
    this.store.dispatch(
      new SetSelectedHduId(this.store.selectSnapshot(DataFilesState.getHduEntities)[$event.value].fileId, $event.value)
    );
  }
}
