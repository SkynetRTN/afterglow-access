...
var WorkbenchDataFileListComponent = /** @class */ (function () {
    function WorkbenchDataFileListComponent(store) {
        ...
        this.nodes$ = this.files$.pipe(operators_1.distinctUntilChanged(function (a, b) { return a && b && a.length == b.length && a.every(function (value, index) { return b[index].id == value.id; }); }), operators_1.switchMap(function (files) {
            var hduEntities = _this.store.selectSnapshot(data_files_state_1.DataFilesState.getHduEntities);
            return rxjs_1.combineLatest.apply(void 0, files.map(function (file) {
                return _this.store.select(data_files_state_1.DataFilesState.getDataFileById).pipe(operators_1.map(function (fn) { return fn(file.id); }), operators_1.distinctUntilChanged(function (a, b) { return a && b && a.name == b.name && a.hduIds == b.hduIds; }), operators_1.map(function (file) {
                    var result;
                    if (file.hduIds.length > 1) {
                        result = {
                            id: file.id,
                            name: file.name,
                            children: file.hduIds.map(function (hduId, index) { return ({
                                id: file.id + "-" + hduId,
                                name: "Channel " + index,
                                children: [],
                                hasChildren: false,
