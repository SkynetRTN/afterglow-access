  ...
  constructor(
    ...
    let reqs = filesArray.map(file => {
      ...
      return from(chunks).pipe(
        concatMap(chunk => {
          return this.dataProviderService.createAsset(this.target.dataProvider.id, `${parentAssetPath}/${file.name}`, file, chunk).pipe(
            tap(v => {
              this.dataSource.data.find(item => item.name == file.name).progress = 100.0*(chunk.chunkIndex+1)/chunk.chunkCount
            })
          )
        }),
        catchError(err => {
